...
@Component({
  ...
  styleUrl: './supplier-invoice.component.scss',
  imports: [
    CommonModule,
    FormsModule,
  ]
...
export class SupplierInvoiceComponent extends BaseComponent implements OnInit {
  ...
  getAllSupplierInvoice() {
    this.supplierInvoiceService.getAllSupplierInvoice().subscribe({
      next: (response: ApiResponse) => {
        debugger
        this.supplierInvoices = response.data;
        this.supplierInvoices = this.supplierInvoices.reverse();
      },
      complete: () => { debugger },
      error: (error: HttpErrorResponse) => {
        debugger;
        console.error('Error fetching supplierInvoices: ', error)
      }
    })
  }
  ...
  getFileUrl(fileName: string): string {
    return this.supplierInvoiceService.viewFile(fileName);
  }
    ...
    if (result) {
      this.supplierInvoiceService.blockOrEnable(invoiceId, 1).subscribe({
        next: () => {
          ...
          this.getAllSupplierInvoice(); // Refresh danh sách
        },
        error: (err) => {
          this.toastService.showToast({
