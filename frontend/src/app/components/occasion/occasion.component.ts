...
export class OccasionComponent extends BaseComponent implements OnInit {
  products: Product[] = [];
  occasion?: Occasion;
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.paramMap.subscribe(params => {
      ...
      if (idParam !== null) {
        this.selectedOccasionId = +idParam; // Chuyển đổi sang số
      } else {
        console.error('Cannot find occasion');
        this.router.navigate(['/']); // Điều hướng về trang chủ nếu không có tham số
        return;
      }
    });
    this.getOccasionById(this.selectedOccasionId);
  ...
  getOccasionById(occasionId: number) {
    debugger;
    this.occasionService.getOccasionById(occasionId).subscribe({
      next: (apiResponse: ApiResponse) => {
        debugger;
        this.occasion = apiResponse.data;
      },
      complete: () => { debugger; },
      error: (error: HttpErrorResponse) => {
        this.toastService.showToast({
          error: error,
          defaultMsg: 'Lấy dữ liệu dịp sản phẩm không thành công',
          title: 'Lỗi'
        })
      }
    })
